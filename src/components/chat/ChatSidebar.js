...
export default function ChatSidebar() {
  ...
  const handleChangeSearch = async (event) => {
    try {
      ...
      if (value) {
        const response = await httpservice.get('/chat/search', {
          params: { query: value }
        });
  ...
  const handleSearchSelect = (conversationId) => {
    setSearchFocused(false);
    setSearchQuery('');
    navigate(`${PATH_CHAT.root}/${conversationId}`);
  };

  const handleSelectContact = (result) => {
    if (handleSearchSelect) {
      handleSearchSelect(result.conversationId);
    }
  };

  const handleModalAddContact = () => {
     setOpenModalAddContact(!OpenModalAddContact)
  }

  const renderContent = (
    <>
    <AddContact onClose={handleModalAddContact} open={OpenModalAddContact} />
      <Box sx={{ py: 2, px: 3 }}>
